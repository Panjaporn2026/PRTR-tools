// ══════════════════════════════════════════════════════
//  ZIP + XLSX raw read/write primitives — ported verbatim from health-reconcile/excel.js
//  (itself ported from accrued-income/index.html). Same xlsx-as-zip format, same techniques.
// ══════════════════════════════════════════════════════
...
// The workbook's FIRST (and, for this file family, only) worksheet, regardless of its tab name --
// GL_Invoice-family files use a human-readable name like "GL Invoice Report", not "Sheet1", so
// callers that don't already know the exact tab name should use this instead of getSheetXml.
...
function parseGridFromXml(xml, sst) {
  ...
  // self-closing (confirmed real bug hit earlier this session), so both forms must be handled.

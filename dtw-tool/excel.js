// ══════════════════════════════════════════════════════
//  ZIP + XLSX raw read/write primitives — ported verbatim from gl-invoice/excel.js
//  (itself ported from health-reconcile/excel.js / accrued-income/index.html). Same xlsx-as-zip
//  format, same techniques.
// ══════════════════════════════════════════════════════
...
async function loadWorkbook(buf) {
  ...
  // other tools can write them in a different order. Parse each tag fully, then pull attributes
...
// The workbook's FIRST worksheet, regardless of its tab name -- some report exports use a
// human-readable name like "GL Invoice Report", not "Sheet1".
function firstSheetPath(wb) {
  var names = Object.keys(wb.sheets);
  if (!names.length) throw new Error('ไม่พบชีตในไฟล์นี้เลย');
...
// JS numbers, everything else stays a string.
async function readSheetGrid(wb, sheetName) {
  var xml = await getSheetXml(wb, sheetName);
  var sst = await getSharedStrings(wb);
  return { aoa: parseGridFromXml(xml, sst), sheetXml: xml };
}

function parseGridFromXml(xml, sst) {
  var aoa = [];
  // Handles both self-closing <row r="N" .../> (common for blank rows) and full
  // <row r="N" ...>...</row> forms -- a row-parsing regex that only matches the full form will
  // silently swallow content through to the NEXT row's closing tag when an intervening row is
  // self-closing, so both forms must be handled.
  var rowRe = /<row\b[^>]*\br="(\d+)"[^>]*(?:\/>|>([\s\S]*?)<\/row>)/g, rm;
  while ((rm = rowRe.exec(xml)) !== null) {
    var rowNum = parseInt(rm[1], 10);
    var rowInner = rm[2];
    if (rowInner === undefined) continue; // self-closing, no cells
    ...
    while ((cm = cellRe.exec(rowInner)) !== null) {
      ...
      if (type === 's' && vMatch) {
        ...
        // Formula cell whose CACHED result is a string (e.g. OUTSOURCE's "Payment due" column,
        // itself an external-link formula resolving to text like "100 Days") -- must be read as
        // text, not run through parseFloat, which would silently truncate "100 Days" to 100.
